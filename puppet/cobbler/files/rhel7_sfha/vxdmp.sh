...
# Backup original lvm.conf file before migration to native VxDMP
...
# Update crashkernel parameter to 256 MB in order to properly load
...
# Set/add selinux=0 parameter (ie disable selinux) on vmlinuz commands in grub.cfg
read -a linenos <<< \$(grep -n vmlinuz /boot/grub2/grub.cfg | awk -F':' '{print \$1}')
for lineno in \${linenos[@]};
do
    sed -i "\${lineno}"'s/selinux=[1-9]/selinux=0/g' /boot/grub2/grub.cfg

    sed -n "\${lineno}"p /boot/grub2/grub.cfg | grep -q 'selinux=0'
    if (( $? == 1 )) ; then
        sed -i "\${lineno}"'s/\(^.*\$\)/\1 selinux=0/' /boot/grub2/grub.cfg
    fi
done
